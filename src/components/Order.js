...
const Order = ({currentOrder, currentTotal}) => {
  ...
  return (
    ...
    {Object.keys(currentOrder).filter(key => key !== "total").map((keyName, index) => {
      ...
      <h5 className='ml-5'>{Math.floor(currentOrder[keyName].quantity * currentOrder[keyName].price).toString()}</h5>
      </div>)
    })}
